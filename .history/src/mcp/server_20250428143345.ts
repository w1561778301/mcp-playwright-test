...
export function createPlaywrightMCPServer(
  ...
  // 注册资源：获取测试报告
  server.resource("test-report", "test-report://{reportId}", async (uri, params: Record<string, string>) => {
    try {
  ...
  // 注册资源：获取API测试报告
  server.resource("api-test-report", "api-test-report://{reportId}", async (uri, params: Record<string, string>) => {
    try {
  ...
  server.resource(
    ...
    async (uri, params: Record<string, string>) => {
      try {
        // 获取测试套件
