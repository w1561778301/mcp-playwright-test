...
export function createHTTPTransport(
  ...
  if (logLevel !== 'none') {
    app.use((_req, res, next) => {
      ...
      res.on('finish', () => {
        ...
        if (logLevel === 'debug') {
          console.log(`${_req.method} ${_req.url} - ${res.statusCode} (${duration}ms)`);
        } else if (logLevel === 'info' && (res.statusCode >= 400 || duration > 1000)) {
          console.log(`${_req.method} ${_req.url} - ${res.statusCode} (${duration}ms)`);
        } else if (logLevel === 'warn' && res.statusCode >= 400) {
          console.warn(`${_req.method} ${_req.url} - ${res.statusCode} (${duration}ms)`);
        } else if (logLevel === 'error' && res.statusCode >= 500) {
          console.error(`${_req.method} ${_req.url} - ${res.statusCode} (${duration}ms)`);
        }
      });
      next();
    });
  ...
  app.get('/server-info', (_req, res) => {
    // 使用扩展的类型避免any
    const serverExt = server as ExtendedMcpServer;

    res.status(200).json({
      // 使用可选链和默认值访问属性
      name: serverExt.name ?? serverMetadata.name,
      version: serverExt.version ?? serverMetadata.version,
      description: serverExt.description ?? serverMetadata.description,
    });
  });
...
function setupStatelessMode(app: express.Application, server: McpServer) {
  app.post('/mcp', (req: Request, res: Response, next: express.NextFunction) => {
