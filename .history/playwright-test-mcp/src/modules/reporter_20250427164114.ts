...
<body>
  ...
  ${report.frontendErrors.map(err => `
    <div class="error">
      <h3>${this.escapeHtml(err.message)}</h3>
      <div class="details">
        ${err.location ? `<p>Location: ${this.escapeHtml(err.location.file)} (line ${err.location.line}, column ${err.location.column})</p>` : ''}
  ...
  ${report.backendErrors.map(err => `
    <div class="error">
      <h3>${this.escapeHtml(err.method)} ${this.escapeHtml(err.apiEndpoint)} - ${err.status} ${this.escapeHtml(err.statusText)}</h3>
      <div class="details">
        <p>Timestamp: ${this.escapeHtml(err.timestamp)}</p>
        ${err.requestPayload ? `
          <h4>Request Payload:</h4>
          <pre>${this.escapeHtml(typeof err.requestPayload === 'string' ? err.requestPayload : JSON.stringify(err.requestPayload, null, 2))}</pre>
        ` : ''}
        ${err.responseBody ? `
          <h4>Response Body:</h4>
          <pre>${this.escapeHtml(typeof err.responseBody === 'string' ? err.responseBody : JSON.stringify(err.responseBody, null, 2))}</pre>
        ` : ''}
      </div>
    </div>
  `).join('')}
