...
export class ReporterModule {
  ...
  generateAPIErrorReport(apiTestResults: APITestResult[]): ErrorReport {
    ...
    for (const result of apiTestResults) {
      if (!result.passed) {
        // Get original endpoint and method from response headers if available
        const originalEndpoint = result.response?.headers?.['x-original-endpoint'];
        const originalMethod = result.response?.headers?.['x-original-method'];
        
        // Create backend error entry from the available response data
        backendErrors.push({
          apiEndpoint: originalEndpoint || `API Test ${result.testCaseId}`,
          method: originalMethod || 'API',
  ...
  ${report.frontendErrors
    ...
    <div class="error">
      ...
      <div class="details">
        ${
          err.location
            ? `<p>Location: ${this.escapeHtml(err.location.file)} (line ${err.location.line}, column ${err.location.column})</p>`
            : ""
        }
