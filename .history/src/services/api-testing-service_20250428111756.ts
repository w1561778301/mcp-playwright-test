...
export class ApiTestingService {
  ...
  async runApiTests(testSuiteId: string): Promise<string> {
    try {
      ...
      const testSuite = JSON.parse(testSuiteContent) as CoreTypes.TestSuite;
...
`;
   ...
   * @returns 测试用例数组
   */
  private async generateTestCasesFromApiSpec(apiSpec: ApiSpec): Promise<CoreTypes.TestCase[]> {
    try {
      // 构建提示
      const prompt = `
