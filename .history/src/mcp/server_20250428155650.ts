...
export function createPlaywrightMCPServer(
  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        const projectPath = (params.path as string | undefined) || projectConfig.codePath;
    ...
    async (params) => {
      try {
        ...
        await playwrightService.launchBrowser({
          browserType: (params.browserType as "chromium" | "firefox" | "webkit") || browserConfig.type,
          headless: (params.headless as boolean | undefined) || browserConfig.headless,
          slowMo: (params.slowMo as number | undefined) || browserConfig.slowMo,
  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        if (params.apiSpec) {
          const apiSpecPath = params.apiSpec as string;
  ...
  // 注册资源：获取API测试报告
  server.resource("api-test-report", "api-test-report", async (uri, _extra) => {
    try {
      // 解析URI路径，获取reportId
      const uriPath = uri.pathname;
      const pathParts = uriPath.split("/");
      ...
      const report = await apiTesting.getReport(reportId);
