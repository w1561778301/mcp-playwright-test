...
export function createPlaywrightMCPServer(options: { port?: number; transport?: "http" | "stdio" } = {}) {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const options = {
          testId: params.testSuiteId,
          environment: params.baseUrl ? { BASE_URL: params.baseUrl } : undefined,
          timeout: params.timeout
        };

        const reportId = await testExecution.executeTest(options);
        return {
          content: [{
            type: "text",
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const options = {
          url: params.baseUrl || "",
          method: "GET", // 默认方法，实际会从测试套件获取
          headers: params.headers
        };

        const reportId = await apiTesting.executeTests(params.testSuiteId, options);
        return {
          content: [{
            type: "text",
            text: JSON.stringify({ success: true, reportId })
