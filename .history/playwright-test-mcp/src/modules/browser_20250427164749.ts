...
export class BrowserModule {
  ...
  private installDependencies(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      if (!this.projectPath) {
        reject(new Error('Project path not set'));
        return;
      }

      console.log('Installing project dependencies...');

      // Fix the type error by ensuring projectPath is not null
      const projectPathString = this.projectPath; // This ensures it's a string

      exec('npm install', { cwd: projectPathString }, (error: Error | null) => {
        if (error) {
