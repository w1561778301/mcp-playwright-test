...
import { TestCase, TestStep, TestSuite, TestType } from './test-generator-service';
...
`;
      ...
      const response = await this.anthropic.messages.create({
        ...
        max_tokens: 4000,
        system: '你是一个擅长解析API文档的专家，请将API规范文档解析为结构化的JSON对象。只返回JSON格式的数据，不要有其他解释或标记。',
        messages: [
          {
            role: 'user',
            content: prompt,
...
`;
      ...
      const response = await this.anthropic.messages.create({
        model: 'claude-3-haiku-20240307',
        max_tokens: 4000,
        system: '你是一个专业的API测试专家，擅长根据API规范生成测试用例。请只返回JSON格式的响应，不要添加任何额外的解释或Markdown格式。',
        messages: [
          {
            role: 'user',
            content: prompt,
