...
export class OpenApiV3Parser implements APIDocumentParser {
      ...
      ["get", "post", "put", "delete", "patch", "options", "head"].forEach((method) => {
        if (pathItem[method]) {
          ...
          endpoints.push({
            ...
            responseSchema: this.extractResponseSchemas(operation, document),
  ...
  async generateTestCases(parsedDocument: ParsedAPIDocument): Promise<APITestCase[]> {
    ...
    const baseUrl = this.extractBaseUrl({ info: parsedDocument.info });
    ...
    parsedDocument.endpoints.forEach((endpoint) => {
      ...
      if (endpoint.responseSchema) {
        const successCodes = Object.keys(endpoint.responseSchema).filter(
          (code) => code.startsWith("2") || code === "default"
        );

        if (successCodes.length > 0) {
          const statusCode = successCodes[0];
          const contentTypes = Object.keys(endpoint.responseSchema[statusCode] || {});

          if (contentTypes.length > 0) {
            assertions.push({
              type: "schema",
              expected: endpoint.responseSchema[statusCode][contentTypes[0]],
