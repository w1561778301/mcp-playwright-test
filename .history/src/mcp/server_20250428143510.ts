...
export function createPlaywrightMCPServer(
  ...
  // 简化版resource实现
  // 注册资源：获取测试报告
  server.resource("test-report", "test-report", async (uri, extra) => {
    try {
      // 解析URI路径，获取reportId
      const uriPath = uri.pathname || uri.path || "";
  ...
  // 注册资源：获取API测试报告
  server.resource("api-test-report", "api-test-report", async (uri, extra) => {
    try {
      // 解析URI路径，获取reportId
      const uriPath = uri.pathname || uri.path || "";
      const pathParts = uriPath.split("/");
  ...
  // 注册资源：获取测试用例
  server.resource("test-case", "test-case", async (uri, extra) => {
    try {
