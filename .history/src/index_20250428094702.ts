...
export class PlaywrightMCPTestSDK implements PlaywrightTestSDK {
  ...
  async runAPITests(testCases: APITestCase[]): Promise<APITestResult[]> {
    ...
    // Store the original test cases along with the results for better reporting
    const testCaseMap = new Map(testCases.map((tc) => [tc.id, tc]));
    const enrichedResults = results.map((result) => {
      const originalTestCase = testCaseMap.get(result.testCaseId);
      if (originalTestCase && result.response) {
        // Store endpoint and method in response headers for reporting
        if (!result.response.headers) {
          result.response.headers = {};
        }
        result.response.headers["x-original-endpoint"] = originalTestCase.endpoint;
        result.response.headers["x-original-method"] = originalTestCase.method;
      }
      return result;
    });

    // Save results for later reporting
    this.apiTestResults = enrichedResults;
