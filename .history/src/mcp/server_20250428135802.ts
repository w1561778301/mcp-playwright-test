...
export function createPlaywrightMCPServer(options: { port?: number; transport?: "http" | "stdio" } = {}) {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        // 创建测试套件并生成测试用例（这里假设创建了一个测试套件后可以通过getTestCases获取）
        const testSuiteId = await testGenerator.createTestSuite({
          name: `${params.testType === "api" ? "API" : "UI"} Tests - ${new Date().toISOString()}`,
          description: params.requirements,
          type: params.testType,
          project: params.projectPath || "",
        });

        // 获取生成的测试用例
        const testCases = await testGenerator.getTestCases(testSuiteId);
  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        // 执行测试并获取报告ID
        const reportId = await testExecution.runTests(options);
  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        // 执行API测试并获取报告ID
        const reportId = await apiTesting.runApiTests(options);
  ...
  // 注册资源：获取测试报告
  server.resource("test-report", "test-report://{reportId}", async (uri, params) => {
    try {
  ...
  // 注册资源：获取API测试报告
  server.resource("api-test-report", "api-test-report://{reportId}", async (uri, params) => {
    try {
      // 获取API测试报告
  ...
  // 注册资源：获取测试用例
  server.resource("test-case", "test-case://{testSuiteId}/{testCaseId}", async (uri, params) => {
    try {
      // 获取测试用例列表
  ...
  // 注册资源：获取控制台日志
  server.resource("console-logs", "console-logs://browser", async (uri) => {
    try {
  ...
  // 注册资源：获取网络请求
  server.resource("network-requests", "network-requests://browser", async (uri) => {
    try {
