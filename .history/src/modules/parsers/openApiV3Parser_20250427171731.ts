...
export class OpenApiV3Parser implements APIDocumentParser {
  ...
  async generateTestCases(parsedDocument: ParsedAPIDocument): Promise<APITestCase[]> {
    ...
    parsedDocument.endpoints.forEach((endpoint) => {
      ...
      const testCase: any = {
        id: `${endpoint.method.toLowerCase()}-${endpoint.path.replace(/\//g, "-")}`,
        description: endpoint.description || `Verify ${endpoint.method} ${endpoint.path} endpoint`,
      ...
      if (endpoint.parameters && endpoint.parameters.path && endpoint.parameters.path.length > 0) {
        endpoint.parameters.path.forEach((param: any) => {
          // Add path parameter placeholder
          if (testCase.endpoint.includes(`{${param.name}}`)) {
            testCase.endpoint = testCase.endpoint.replace(`{${param.name}}`, `:${param.name}`);
          }
        });
      }
      ...
      if (endpoint.headers && Object.keys(endpoint.headers).length > 0) {
        Object.keys(endpoint.headers).forEach((headerName) => {
          // Add header value
          const header = endpoint.headers[headerName];
          if (header.schema && (header.schema.default !== undefined || header.schema.example !== undefined)) {
            testCase.headers[headerName] = header.schema.default || header.schema.example;
          } else {
            testCase.headers[headerName] = "test";
          }
        });
      }
      ...
      if (["POST", "PUT", "PATCH"].includes(endpoint.method)) {
        ...
        if (endpoint.requestExamples && endpoint.requestExamples.length > 0) {
          testCase.body = endpoint.requestExamples[0].value;
          if (endpoint.requestExamples[0].contentType) {
            testCase.headers["Content-Type"] = endpoint.requestExamples[0].contentType;
          }
        } else if (endpoint.requestSchema && Object.keys(endpoint.requestSchema).length > 0) {
          // Use the first content type available
          const contentType = Object.keys(endpoint.requestSchema)[0];
          if (contentType) {
            testCase.headers["Content-Type"] = contentType;
      ...
      // Add schema validation assertion if response schema available
      if (endpoint.responseSchemas) {
        const successCodes = Object.keys(endpoint.responseSchemas).filter((code) => code.startsWith("2"));
        if (successCodes.length > 0) {
          const successCode = successCodes[0];
          const contentTypes = Object.keys(endpoint.responseSchemas[successCode] || {});
          ...
          if (contentTypes.length > 0) {
            testCase.assertions.push({
              ...
              target: "schema",
              operator: "matches",
              value: endpoint.responseSchemas[successCode][contentTypes[0]],
            });
          }
        }
      }
