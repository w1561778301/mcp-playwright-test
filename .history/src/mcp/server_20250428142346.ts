  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        if (params.apiSpec && fs.existsSync(params.apiSpec)) {
          apiSpecContent = fs.readFileSync(params.apiSpec, "utf-8");
        }

        // 调用generateFromText方法生成测试用例
        const result = await testGenerator.generateFromText(params.requirements, params.testType, apiSpecContent);
