...
export class ApiTestingService {
  ...
  constructor(storageDir?: string, mcpClient?: any) {
    ...
    // 设置API基础URL
    this.baseApiUrl = projectConfig.apiBaseUrl;
  ...
  public async generateTests(
    apiSpecPath: string = projectConfig.apiDocPath,
    options: {
      endpoints?: string[];
      methods?: string[];
    ...
    try {
      // 处理相对路径
      if (!path.isAbsolute(apiSpecPath)) {
        apiSpecPath = path.resolve(process.cwd(), apiSpecPath);
      }
